...
3.1. ¿Los datos contienen ceros o elementos vacáos? ¿Cómo gestionaráas cada uno de estos casos?
...
train <- select(train, -Name, -Fare, -Ticket )
test  <- select(test, -Name, -Fare, -Ticket )
head (train)
```
...
Llegados a este punto debemos decidir cómo manejar estos registros que contienen valores desconocidos para algun campo. Una opción podrá ser eliminar esos registros que incluyen este tipo de valores, pero ello supondráa desaprovechar información.  
...
modelo1 <- lm(Survived ~ train.Age + train.Sex + train.Embarked + train.sibSp + train.Parch + train.Pclass, data = train)
modelo2 <- lm(Survived ~ train.sibSp + train.Sex + train.Embarked + train.Parch + train.Pclass, data = train)
modelo3 <- lm(Survived ~ train.Age + train.Sex + train.Embarked + train.Parch + train.Pclass, data = train)
modelo4 <- lm(Survived ~ train.Age + train.Sex + train.Embarked + train.sibSp + train.Pclass, data = train)
modelo5 <- lm(Survived ~ train.Age + train.Sex + train.Embarked + train.sibSp + train.Parch , data = train)
modelo6 <- lm(Survived ~ train.Age + train.Sex + train.Embarked + train.FamilySize + train.Pclass, data = train)
modelo7 <- lm(Survived ~ train.Sex + train.Embarked + train.FamilySize + train.Pclass, data = train)
